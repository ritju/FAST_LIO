  ...
  :feature_enabled(0), lidar_type(AVIA), blind(0.01), point_filter_num(1)
{
  ...
  jump_down_limit = 8.0;
  cos160 = 160.0;
  edgea = 2; ////三个连续点，中间到两边点的距离比超过两倍则认为遮挡
  edgeb = 0.1; //三个连续点，中间到两边点的距离比大于0.1m则认为遮挡
...
void Preprocess::oust64_handler(const sensor_msgs::msg::PointCloud2::ConstPtr &msg)
{
  ...
  {
    double time_stamp = tf2_ros::timeToSec(msg->header.stamp);
    // cout << "===================================" << endl;
...
void Preprocess::velodyne_handler(const sensor_msgs::msg::PointCloud2::ConstPtr &msg)
{
    ...
    pcl::fromROSMsg(*msg, pl_orig); //从msg获取数据
    int plsize = pl_orig.points.size();
    if (plsize == 0) return;
    pl_surf.reserve(plsize);

    /*** These variables only works when no point timestamps given ***/
    double omega_l = 0.361 * SCAN_RATE;       // scan angular velocity 例如：10Hz 1s转360度 1ms转0.36度
    ...
    if(feature_enabled)
    {
      ...
      for (int i = 0; i < plsize; i++)
      {
        ...
        added_pt.normal_y = 0;
        added_pt.normal_z = 0;
        int layer  = pl_orig.points[i].ring; //线程数
        if (layer >= N_SCANS) continue;
        added_pt.x = pl_orig.points[i].x; // 点云x轴坐标
        added_pt.y = pl_orig.points[i].y; // 点云y轴坐标
        added_pt.z = pl_orig.points[i].z; // 点云z轴坐标
        added_pt.intensity = pl_orig.points[i].intensity; // 点云强度
    ...
    else
    {
      ...
      for (int i = 0; i < plsize; i++)
      {
        ...
        added_pt.z = pl_orig.points[i].z; // 点云z轴坐标
        added_pt.intensity = pl_orig.points[i].intensity;
        added_pt.curvature = pl_orig.points[i].time * time_unit_scale;  // curvature unit: ms // cout<<added_pt.curvature<<endl; 使用曲率作为每个激光点的时间
        ...
        {
          // 只有点在最小距离阈值之外，才将当前点认为是有用的点，加入到pl_surf队列中
...
void Preprocess::give_feature(pcl::PointCloud<PointType> &pl, vector<orgtype> &types)
{
  ...
  for(uint i=head; i<plsize2; i++)
  {
    ...
    if(plane_type == 1) //返回1一般默认是平面
    {
      ...
      //最开始last_state=0直接跳过 之后last_state=1
  ...
  for(uint i=head+3; i<plsize2; i++)
  {
    ...
    //前一个点是正常点 && 下一个点在OA线上 && 当前点与后一个点的距离大于0.0225m && 当前点与后一个点的距离大于当前点与前一个点距离的四倍
    //这种边缘点像是7字形这种的边缘？
    if(types[i].edj[Prev]==Nr_nor && types[i].edj[Next]==Nr_zero && types[i].dista>0.0225 && types[i].dista>4*types[i-1].dista)
    {
      if(types[i].intersect > cos160) //角MAN要小于160度 不然就平行于激光了
      {
        ...
        {
          types[i].ftype = Edge_Jump; //有跨越的边
        }
      }
    }
    //前一个点在OA上 && 后一个点正常 && 前一个点与当前点的距离大于0.0225m && 前一个点与当前点的距离大于当前点与后一个点距离的四倍
...
int Preprocess::plane_judge(const PointCloudXYZI &pl, vector<orgtype> &types, uint i_cur, uint &i_nex, Eigen::Vector3d &curr_direct)
{
  ...
  for(;;)
  {
    if((i_cur >= pl.size()) || (i_nex >= pl.size())) break;//索引超出所有点的个数直接BREAK
  ...
  for(uint j=i_cur+1; j<i_nex; j++)
  {
    ...
    //向量AB叉乘向量AC，叉乘的几何意义是如果以向量AB和AC为边构成一个平行四边形，那么这两个向量外积的模长与这个平行四边形的面积相等。
...
bool Preprocess::edge_jump_judge(const PointCloudXYZI &pl, vector<orgtype> &types, uint i, Surround nor_dir)
{
  ...
  double d1 = types[i+nor_dir-1].dista; //当前点与前一点的距离，或当前点与后一点的距离
  double d2 = types[i+3*nor_dir-2].dista;//前一点和跟前一点的距离，或后一点与更后一点的距离
  double d;
  ...
  //三个连续点，中间到两边点的距离比超过两倍，或差大于0.1m则认为遮挡
